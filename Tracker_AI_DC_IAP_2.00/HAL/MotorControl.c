...
void TIM3_IRQHandler()
{
    if(TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
    {
        TIM_ClearITPendingBit(TIM3, TIM_IT_Update); 
        _pwm_count ++;
				if(_pwm_count>(6776*1.1f))  //频率改为1.1K，所以时间计数也要换算，电机缓启动时间定为5.6S，这样会比较平缓
        {       
					  TimerStop();
        ...
        if(_pwm_count<(968*1.1f))  //这里初步定为三个上升斜率      
				{
					  _pwm_test = _pwm_count *COUNT_PER_MS1 * 2000.0f / (968.0f*1.1f);
				}
        else if(_pwm_count<(2904*1.1f))
				{
					  _pwm_test = _pwm_count * COUNT_PER_MS2 * 2000.0f / (1936.0f*1.1f);
				}
				else if(_pwm_count<(4840*1.1f))
				{
					  _pwm_test = _pwm_count * COUNT_PER_MS3 * 2000.0f / (1936.0f*1.1f);
				}
				else if(_pwm_count<(6776*1.1f))  //继续维持一段时间，试下来能有效减小最后一下的冲击电流
				{
					  _pwm_test = (4840*1.1f) * COUNT_PER_MS3 * 2000.0f / (1936.0f*1.1f);
				    if(GlobalVariable.Motor[0].Motorcurrent > 1.0f || GlobalVariable.Motor[0].Motorcurrent < -1.0f)
				    {
					      _pwm_test = (4840-2)*1.1f * COUNT_PER_MS3 * 2000.0f / (1936.0f*1.1f);
				    }
				}						
